...
export function RevenueChart({ series }: { series: RevenueRow[] }) {
  const chartUid = useId().replace(/:/g, "");
  const idAreaGradient = `revenue-area-grad-${chartUid}`;
  const [periodDays, setPeriodDays] = useState<PeriodDays>(60);

  const chartRows = useMemo(
    () => series.slice(-periodDays),
    [series, periodDays],
  );

  const hasAny = chartRows.some((r) => r.revenue > 0);
      ...
      <CardContent>
        {!hasAny ? (
          <p className="flex h-60 items-center justify-center text-sm text-muted-foreground">
            No booked orders in this window.
          </p>
        ) : (
          <ChartContainer
      ...
      <CardFooter className="flex items-center justify-between">
        <div className="flex items-center gap-1 text-muted-foreground text-xs">
          {hasAny ? (
            ...
            <p>Daily booked from non-cancelled orders.</p>
          )}
        </div>
        <Button
          className="text-muted-foreground"
          nativeButton={false}
          render={<Link href="/office/reports" />}
          size="xs"
          variant="ghost"
        >
          View report
